...
The human-in-the-loop calibration workflow proved essential: automated detection alone achieved approximately 75\% precision, improved to $>95\%$ after three interactive correction rounds. This suggests that for campus-scale applications, semi-automated approaches combining algorithmic detection with targeted human verification are more practical than fully autonomous methods.
...
We demonstrated a complete pipeline for hyperlocal pollen dispersion modeling, from satellite-based source identification to physics-based atmospheric transport to actionable exposure assessments. The system resolves concentration gradients at 5-meter scale across a 25-hectare campus with 1,018 individually mapped pollen sources, enabling allergy-sensitive students to make informed daily routing decisions based on current meteorological conditions and seasonal phenology.

\section*{Data Availability}

Source code and campus data are available at the project repository. The tree inventory is derived from the PAUSD 2009 arborist survey supplemented by satellite detection.
