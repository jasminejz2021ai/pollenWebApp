...
Both tree and building detections undergo iterative refinement: automated detection proposes candidates, a reviewer deletes false positives and adds or reshapes footprints through an interactive map editor (adjusting each building's name, floor count, footprint, and rotation), and the corrected set is persisted to the detection cache that the server serves. For each building the height is taken as (floors $\times$ 3~m), which sets the indoor exclusion volume.
...
On the Gunn campus the detection pipeline maps roughly 1{,}000 trees across six allergenic genera (Coast Live Oak, Coast Redwood, Valley Oak, Pine, Western Sycamore, and Chinese Elm) together with the campus buildings; the exact counts evolve as the set is refined through the interactive editor. On the larger Stanford core, tiled SAM detection identifies on the order of $10^3$ building rooftops and $\sim$$10^4$ tree canopies, giving comprehensive coverage of the full campus. Detection and imagery are cached per campus, so no machine-learning model runs on the live server.
